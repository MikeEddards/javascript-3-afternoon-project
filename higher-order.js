...
//Code Here
let totalPopulation = populations.reduce(function(cur, next){
  return cur + next
})
...
let orderTotals = orders.map(function(orders){
    let tax = orders.tax + 1
    let totals = orders.price * tax
    return totals
})
...
*/
total = 0
let bobsTotal = purchases.filter(function(comp){
    
    
    if(comp.owner.includes('Bob')){
